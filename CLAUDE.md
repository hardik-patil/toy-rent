...
| 2026-09-01 | infra (K8s) | STARTUP.md's whole scale-up flow assumed a "stopped" cluster (namespaces/PVCs already exist, everything scaled to 0) — none of it applied to a genuinely fresh cluster (no namespaces, no PVCs, node age matched the container runtime's own uptime) | STARTUP.md was written for the stop/start cycle documented in SHUTDOWN.md and never covered first-time setup from nothing | Did the full first-time bring-up manually: `k8s/namespace.yaml` + `k8s/network-policy.yaml` first, then every `k8s/infra/*/*.yaml` (each is self-contained — Secret/ConfigMap/StatefulSet-or-Deployment/Service/Job all in one file, `kubectl apply -f` per file, no separate init step needed), then monitoring, then app services one at a time. Worth adding a "first-time setup" section to STARTUP.md distinct from its stop/start section |
| 2026-09-01 | api-gateway, toy-service, booking-service (K8s) | `docker build` failed with `ImagePullBackOff` / `ErrImagePull` — none of `toyrental/toy-service:1.0.10`, `toyrental/booking-service:1.0.10`, `toyrental/api-gateway:1.0.7` existed locally on a fresh checkout | The images are never pushed to a registry — each Dockerfile expects a pre-built jar (`./mvnw package -DskipTests`) plus a `newrelic.jar` (gitignored, not in the repo) already sitting in the build context, built locally once and cached in the host's Docker image store, which a fresh clone/environment obviously doesn't have | Set `JAVA_HOME` (needed for `./mvnw`, not set by default in this shell), ran `./mvnw package -DskipTests` in each of the 3 service dirs, downloaded the New Relic Java agent zip from `download.newrelic.com` and copied `newrelic.jar` into all 3 service dirs, then `docker build` each. The built images were visible to the K8s cluster's nodes immediately (same Docker daemon/image store — Docker Desktop's Kubernetes shares it, unlike a standalone `kind` cluster which would need `kind load docker-image`) |
| 2026-09-01 | toy-service, booking-service, api-gateway (K8s) | Re-enabling the New Relic agent via `kubectl set env deployment/<name> -n toy-rental JDK_JAVA_OPTIONS-` (trailing dash, no value) silently didn't work — agent never loaded on any of the 3 services after a real license key was patched in, zero New Relic log lines anywhere despite the rollout succeeding | Wrong mental model, now corrected in STARTUP.md and `scripts/`: `kubectl set env deployment/x KEY-` does not "fall back to the manifest's value" — plain Kubernetes has no such override/base layer. It just deletes the env var key outright from the live Deployment spec. The earlier `kubectl set env deployment/x JDK_JAVA_OPTIONS=""` had already overwritten (not shadowed) the manifest-applied value, so there was nothing to "fall back" to | Re-`kubectl apply -f k8s/services/<service>/<service>.yaml` for all 3 — `kubectl apply`'s three-way merge (via the `last-applied-configuration` annotation) correctly restores fields removed by other means, unlike `kubectl set env ... KEY-`. Fixed `scripts/enable_new_relic.py` and its docs to match |
| 2026-09-01/02 | all services (K8s) | Repeatedly, any time a Deployment/StatefulSet-touching `kubectl apply`/`set env`/`rollout restart` recreated a pod, whatever was port-forwarded to it (Postgres, Couchbase, toy-service, booking-service, Prometheus — all hit at least once) silently stopped working. Each looked like a fresh, unrelated failure (DBeaver hangs, curl connection refused, "no metrics in Prometheus") until traced back to the same cause | `kubectl port-forward`, even against a Service (`svc/x`), binds to one specific backend pod's sandbox at connect time — it does not follow the Service to a replacement pod when the original is recreated. This is basic `kubectl port-forward` behavior, not a bug, but easy to forget mid-session when many unrelated `kubectl apply`s are happening | No structural fix applied — just re-run the same `kubectl port-forward` command after anything that could have recreated a pod. Worth checking `netstat -ano \| grep 127.0.0.1:<port>` (or just retrying the curl/connection) as a first troubleshooting step any time a previously-working local connection suddenly fails, before assuming a deeper problem |
| 2026-09-01 | infra (K8s, Windows/Git Bash) | `kubectl cp local-file.sql infra/postgres-0:/tmp/x.sql` and `kubectl exec ... -- psql -f /tmp/x.sql` both failed — the former with `pods "infra\postgres-0;C" not found`, the latter with psql trying to open `C:/Users/USER/AppData/Local/Temp/seed_toys_bulk.sql` (a Windows path) instead of the pod-local `/tmp/seed_toys_bulk.sql` | Git Bash's MSYS2 layer auto-converts any argument that looks like a Unix path into a Windows path before exec'ing the real binary — including the pod-local path half of a `kubectl cp`/`kubectl exec -f` argument, which was never meant to be touched | Prefix the command with `MSYS_NO_PATHCONV=1` (e.g. `MSYS_NO_PATHCONV=1 kubectl cp ...`) — disables the auto-conversion for that one invocation. Needed on both the `kubectl cp` destination and the `kubectl exec -f` remote path separately |
| 2026-09-02 | monitoring (K8s) | Prometheus's `cadvisor`/`kubelet` scrape jobs sat permanently `down` (`connect: connection refused`) all session, so `container_cpu_usage_seconds_total`/`container_memory_*` per-pod queries returned nothing | `k8s/infra/prometheus/prometheus.yaml` hardcoded the node's kubelet address as a static IP (`172.20.0.4:10250`), stale from some earlier cluster instance — this fresh cluster's actual node IP (`172.18.0.2`) is assigned by Docker/kind at creation time and has no reason to match a previously-hardcoded value | Replaced `static_configs` with `kubernetes_sd_configs` (`role: node`), which resolves each node's real address dynamically via the API server at scrape time using the same in-cluster auth/RBAC already granted — survives future cluster rebuilds instead of going stale again. See commit `dc0b045` |
| 2026-09-02 | frontend (Windows) | `npm install`/`npm run dev` failed — `node`/`npm` not found on PATH at all | Node.js was never installed on this machine | Installed via `winget install OpenJS.NodeJS.LTS` — needed a real UAC elevation prompt the automated session couldn't click through itself, so the user ran it. Separately: even after install succeeded, this session's already-running shell still couldn't see `node`/`npm` on PATH — a new install's PATH change is only visible to *new* processes spawned by something that re-reads the registry (e.g. Explorer), not to child processes of an already-running parent (this harness). Worked around by locating `node.exe`/`npm.cmd` directly under `C:\Program Files\nodejs\` and prepending that to `PATH` per-command rather than relying on a bare `node`/`npm` call |
| 2026-09-02 | infra (K8s) | `kubectl top pod`/`kubectl top node` return `error: Metrics API not available` — confirmed both at the very start of this session (before anything was deployed) and again hours later, so not something this session broke | No `metrics-server` has ever been installed in this cluster — not a default component of a Docker Desktop/kind-based local cluster, unlike a managed cloud cluster | Not yet fixed — offered to install `metrics-server` (needs `--kubelet-insecure-tls` for this self-signed local cluster) but not done as of this writing. `docker stats` (node-wide only, not per-pod) or the now-fixed Prometheus `cadvisor` job (real per-pod CPU/memory, see the row above) work as substitutes today |
| 2026-09-05 | monitoring (K8s) | Every panel on the `toyrental-overview` Grafana dashboard (and a UI-saved copy, "ToyRental Platform — 15 min") showed no data, even though the Prometheus datasource and all scrape targets were healthy | `grafana/dashboards/toyrental-overview.json` (and its inline copy in `k8s/infra/grafana/grafana.yaml`) is a dashboard exported via Grafana's "Export for sharing externally", which leaves every panel's datasource as the literal placeholder string `${DS_PROMETHEUS}` for the *importer* to substitute — that substitution only happens through the UI import flow, never for a dashboard loaded straight off disk by the file provisioner, so `${DS_PROMETHEUS}` was passed through unresolved and every panel referenced a nonexistent datasource UID. Compounding this: the datasource provisioning config had never pinned an explicit `uid`, so Grafana auto-assigned one (`PBFA97CFB590B2093`, its legacy default for datasource id=1) that isn't reproducible from the YAML alone | Added `uid: PBFA97CFB590B2093` to the datasource entry in `k8s/infra/grafana/grafana.yaml` (matching the value already persisted in the `grafana-data` PVC, so no data source rename/migration was needed) and replaced all 63 `${DS_PROMETHEUS}` occurrences in that file with the same literal UID; separately gave `grafana/provisioning/datasources/datasource.yml` (docker-compose) and `grafana/dashboards/toyrental-overview.json` an explicit `uid: prometheus` / matching literal, since that stack's Grafana volume has no pre-existing datasource row to conflict with. Also patched the UI-saved dashboard copy directly via `POST /api/dashboards/db` since it's not file-provisioned and wouldn't have picked up the manifest fix. **Caution if reapplying district-wide:** setting a datasource's `uid` via provisioning to a value that doesn't match an already-persisted row of the same `name` fails Grafana startup outright (`Datasource provisioning error: data source not found`, crash-loops on every restart) — confirmed by trying `uid: prometheus` first and having to roll back to the existing auto-assigned UID |
| 2026-09-05 | loadtest (JMeter) | A full 15-minute `ToyRentalMixed-60-tps.jmx` run recorded ~18,500 `login` samples and *zero* samples for every other request (BROWSE, toy detail, booking) — every one of the 60 threads spent the entire run stuck re-logging in, never reaching the browse/detail/booking traffic-mix | The `While Controller-relogin-every-10min`'s condition, `${__jexl3(System.currentTimeMillis() >= Long.parseLong(vars.get("tokenExpiry")))}`, calls two static Java methods from inside `__jexl3()`. Confirmed via an isolated JSR223 probe that JMeter 5.6.3's bundled Commons JEXL3 silently fails on *any* static-method call from `__jexl3` — no exception logged anywhere, the call just resolves to an empty string — so the whole condition never evaluated to the literal string `"false"` WhileController checks for, and the loop never exited. `vars.get(...)` alone (no static call) evaluates fine in the same `__jexl3` context, which is why the sibling `${__jexl3(vars.get("bookingid") != "NOT_FOUND")}` elsewhere in the same file is unaffected | Changed the condition to `${__groovy(System.currentTimeMillis() >= Long.parseLong(vars.get("tokenExpiry")),)}` — Groovy (already used elsewhere in this plan's JSR223 elements) has no such sandboxing. Verified with an isolated probe (exactly 1 login instead of an infinite loop) and then a live 5-thread/60s smoke run against the real cluster: BROWSE/detail/booking samples all appeared in the expected ~80/12/8% proportions with only 5 logins total. Rule of thumb for this file going forward: never call a static Java method (`ClassName.method(...)`) from `__jexl3`/`__jexl2` — use `__groovy` instead |
