...
| 2026-08-22 | infra (K8s) | `couchbase-0` never reached Ready — OOMKilled in ~5-7 seconds flat at every tested container memory limit (1Gi/2Gi/4Gi), even after the whole Docker Desktop VM's memory was raised from 8GB to 12GB | Not fully diagnosed. A too-short liveness probe `initialDelaySeconds` was fixed first (60s → 120s) but didn't resolve it; ruling out node-wide memory pressure (by raising VM memory with no change in outcome) proved the container's own cgroup limit was the binding constraint, but why Couchbase Server's baseline footprint on this specific image/arm64/Docker-Desktop-kind combination exceeds 4Gi just to boot remains unknown | None yet — scaled to 0 replicas and deferred (manifest still specifies `replicas: 1` as the intended default); the app services' documented Couchbase-unavailable fallback paths were relied on instead to validate the rest of the platform |
